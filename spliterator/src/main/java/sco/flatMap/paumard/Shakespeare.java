...
public class Shakespeare {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        ...
        Set<String> shakespeareWords = 
            Files.lines(Paths.get(ClassLoader.getSystemResource("m2/words.shakespeare.txt").toURI()))
                .map(word -> word.toLowerCase())
                .collect(Collectors.toSet());
        
        Set<String> scrabbleWords =
            Files.lines(Paths.get(ClassLoader.getSystemResource("m2/ospd.txt").toURI()))
                .map(word -> word.toLowerCase())
                .collect(Collectors.toSet());
